...
## 15.4 算法在等高线图上的效果比较
==SGD、动量算法、RMPSProp、adam的原理、优缺点(与SGD相比，1、动量使loss下降更平滑，没有突然的转向；2、根据累积平方梯度来自适应调整学习率，使每个自变量的学习率不同，loss下降更平缓。最终的结果就是loss收敛更快)
效果区别==
...
- Momentum算法，学习率只有0.1，每次继承上一次的动量方向，所以会以比较平滑的曲线方式前进，不会出现突然的转向。==(每次更新会考虑历史梯度和当前梯度，用以调整方向，使下降平滑；同时调整大小，方向变化则放慢和方向不变则加快)==。
