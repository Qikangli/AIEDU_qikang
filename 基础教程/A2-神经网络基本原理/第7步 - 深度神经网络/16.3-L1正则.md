...
==L2正则化，所有的w都小，有用、无用的特征都可能被忽略；L1正则化，有用特征w较大，无用特征的w为0==
...
### 16.3.5 反向传播的变化
==L1的导数：假设正数，导数为1，假设负数，导数为-1，即取符号==
...
可以看到L1的模型权重非常稀疏（趋近于0的数量很多）。那么==参数稀疏有什么好处呢==？有两点：

1. 特征选择(Feature Selection)：==特征的自动选择，学习去除噪声、非全局特征(权重置零)，避免过拟合==
...
2. 可解释性(Interpretability)：==对于分类问题，假如所有特征的w都很小，无法选出重点特征，所有特征共决定分类结果；假如只有少数特征的w不为0(稀疏)，则可以选出该分类问题的重点决策特征，用以针对性的治疗==
