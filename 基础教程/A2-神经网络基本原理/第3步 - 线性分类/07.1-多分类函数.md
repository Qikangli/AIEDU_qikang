...
## 7.1 多分类函数
==多分类函数softmax的公式、含义(某个样本分别属于三个类别的概率)
logistic函数和softmax函数的关系
正向传播：softmax的交叉熵函数(二分类是这个标准形式的特例)
图像分割与图像分类的区别
反向传播softmax+交叉熵的导数公式
如何防止softmax计算时指数溢出==
...
也就是说，在（至少）有三个类别时，通过使用Softmax公式计算它们的输出，比较相对大小后，得出该样本属于第一类，因为第一类的值为0.879，在三者中最大。注意这是对一个样本的计算得出的数值，而不是三个样本，亦即==Softmax给出了某个样本分别属于三个类别的====概率==。
...
#### 损失函数计算
==交叉熵损失函数的标准形式就是求和(y*lna)；而二分类的损失函数是这个形式的特例==
...
==我理解的图像分割和上述简单神经网络的区别是：上述神经网络输入的空间维度为1*1，然后多个通道(多个维度的特征，如经纬度)，输出多个1 *1的feature map值(即多个z值，每一个对应一个类别)，接softmax变为在每个类别上的概率；然后图像分割就是空间维度不为1，其他不变==
...
### 7.1.4 代码实现
==实际实现softmax，为了防止指数计算溢出，会对数据减去最大值之后，做softmax(与不减最大值是一样的)==
