...
==input的梯度计算：1、回传误差矩阵与180度旋转卷积核的卷积；2、为了得到input的大小，回传误差矩阵需要padding；3、stride不为1怎么求？0插值，然后使用上述stride为1的方式计算；4、有多个kernel怎么办？每个kernel的结果之和
卷积核权重的梯度计算：w的梯度等于输入与回传误差矩阵的卷积
偏置的梯度计算：等于feature map的回传误差矩阵元素之和==
...
==方法：对回传的误差矩阵进行0插值，变为stride=1的误差矩阵；然后使用stride=1的回传公式==
...
==偏置的梯度直接等于feature map的回传误差矩阵元素之和==
